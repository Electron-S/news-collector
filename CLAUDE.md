...
2. `node mailer.js <리포트 경로>` — 작성된 리포트를 **HTML 이메일**로 발송(Gmail SMTP).

수동으로 단계를 나눠 실행할 수도 있다:

```bash
REPORT=$(node collect.js) && node mailer.js "$REPORT"
node mailer.js "$REPORT" --dry-run   # 전송 없이 HTML 미리보기(reports/email-preview.html)
```

전송은 **이메일 1통**(텔레그램 아님). `.env` 의 `SMTP_USER`/`SMTP_PASS`(Gmail 앱 비밀번호)·`MAIL_TO` 필요.
...
- **피드백 학습(feedback.js)**: 이메일은 인터랙티브 버튼이 없으므로 **주간 설문**으로 받는다. 이메일 푸터의 설문 링크(`config.feedback.surveyUrl`)로 관심/비관심 주제를 응답하고, 폼 응답 시트를 '웹에 CSV 게시'한 공개 URL(`responsesCsvUrl`)을 `feedback.js`가 인증 없이 읽어 `state/weights.json`(키워드 가중)을 갱신. 학습값은 HN 랭킹 가중(`interestBoost`)·LLM 프롬프트(선호/비선호 주입)에 반영. `responsesCsvUrl` 미설정 시 링크만 노출(학습 idle). config.interests 파일은 불변.
- **주간 리캡(P2, weekly.js)**: `state/history.jsonl`(노출 항목 이력)에서 최근 `weekly.windowDays`일 상위 항목을 카테고리별로 모아 월요일 별도 **이메일** 전송(점수+피드백 가중 랭킹). 전송은 `mailer.js` 재사용.
- **시장 감성(P6)**: 구독 채널 경제 항목에 LLM이 강세/약세/중립을 매겨 📈/📉/➖ 로 표기.
- **실패 알림**: 핵심 소스 실패 시 본문 상단에 `⚠️ 일부 수집 실패: …` 표기. 수집 크래시·메일 발송 실패 등 하드 실패는 `run.sh` 가 `alert.js` 로 **이메일** 관리자 알림을 보낸다.
...
- 본문은 `### 💻 IT/기술` / `### 📈 경제/투자` 2개 카테고리로 구성(구독 채널 소식은 각 카테고리 안에 `#### 📨` 소블록으로 포함). `mailer.js` 가 최근 실행 섹션을 **HTML 이메일 1통**으로 렌더(두 섹션 모두 포함). 마크다운→HTML 변환은 `renderEmail` 순수 함수.
- 기사 제목과 URL을 세트로 출력(이메일에선 제목이 링크). 마켓·뉴스류는 1행으로 기재.
